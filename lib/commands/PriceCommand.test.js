...
describe('PriceCommand', function () {
  ...
  describe('.canHandleDelayedResponse', () => {
    it('can handle delayed "i pay 7.5"', () => {
      const command = new PriceCommand(requestFor('i pay 7.5'))
      return expect(command.canHandleDelayedResponse()).to.be.true
    })
  })

  describe('.handleDelayedResponse', () => {
    describe('handles delayed "i pay 7.5"', () => {
      it('when user has no orders', () => {
        ...
        const command = new PriceCommand(requestFor('i pay 7.5'), stubOrdersRepository)
        return expect(command.handleDelayedResponse()).to.eventually.equal('You haven\'t ordered anything.')
      })
      it('when user has ordered a pizza', () => {
        ...
        const command = new PriceCommand(requestFor('i pay 7.5'), stubOrdersRepository)
        return expect(command.handleDelayedResponse()).to.eventually.equal('Set your order for "pizza" at 7.5.')
      })
    })
  })
