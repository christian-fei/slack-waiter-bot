...
describe('CommandHandler', function() {
  ...
  it('/waiter i pay 7.5', () => {
    return userOrdersPizza()
    .then(userPays75)
    .then(expectUserToHaveOrderedPizzaFor75)
  })
  ...
  function handleDelayedReply(text, user_name, channel_id) {
    return CommandHandler.handleDelayedReply({text: text, user_name: user_name, user_id: user_name, channel_id: channel_id})
  }
  ...
  function userPays75() {
    ...
    .then(() => {
      return expect(
        handleDelayedReply('i pay 7.5', user_1, channel_1)
      ).to.eventually.eql('Set your order for "a pizza" at 7.5.')
    })
  }
  ...
  function expectUserToHaveOrderedPizzaFor75() {
    return expect(
      handle('show orders', user_1, channel_1)
    ).to.eventually.eql(`Your orders, ${user_1}:\n - a pizza (7.5)\n`)
  }
